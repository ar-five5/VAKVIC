...
export async function seedStarterData() {
  ...
  for (const asset of STARTER_ASSETS) {
    ...
    seededAssets += result.rowCount;
  }

  const { rows: dbAssets } = await query(
    'SELECT asset_id, ticker_symbol FROM assets WHERE ticker_symbol = ANY($1::text[])',
    [STARTER_ASSETS.map((asset) => asset.ticker)]
  );
  const idByTicker = new Map(dbAssets.map((asset) => [asset.ticker_symbol, asset.asset_id]));

  let seededPrices = 0;
  for (const [index, asset] of STARTER_ASSETS.entries()) {
    const assetId = idByTicker.get(asset.ticker);
    if (assetId) {
      seededPrices += await seedStarterPrices(asset, assetId, index);
    }
  }

  return { seededAssets, seededPrices };
}
